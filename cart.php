...
<body>
    ...
    <section class="cart_area padding_top1">
        <div class="container">
                        ...
                        ?>
                                <table class="table">
                                    ...
                                    <tbody>
                                        <?php
                                        $tongcong = 0;
                                        $products = selectAll("SELECT * FROM ctdonhang WHERE id_donhang=$idDh");
                                        ...
                                        <textarea name="diachi" id="" cols="70" rows="4"
                                            placeholder="Nhập địa chỉ nhận hàng"
                                            required><?= htmlspecialchars($diachitaikhoan) ?></textarea>
                                    </div>
                                            </td>
                                            <td></td>
                                            <td>
                                ...
                                <div class="checkout_btn_inner float-right">
                                    ...
                                    <input class="btn_1" type='submit' name="capnhatgiohang" value="Cập nhật giỏ hàng" style="border: none" />
            ...
            <?php
                        ...
                        if (isset($_GET['removeproduct'])) {
                            // foreach (selectAll("SELECT * FROM ctdonhang WHERE id_sanpham={$_GET['removeproduct']} AND id ={$idDh}") as $item) {

                            //     foreach (selectAll("SELECT * FROM sanpham WHERE id={$item['id_sanpham']}") as $prod) {
                            //         $total = $prod['soluong'] + $item['soluong'];
                            //     }
                            //     exSQL("UPDATE `sanpham` SET `soluong`={$total} WHERE id = {$item['id_sanpham']}");
                            // }
        ...
        if (isset($_POST["dathang"])) {
            $diachi = $_POST["diachi"];
            selectall("UPDATE donhang SET diachi='$diachi', tongtien= $tongcong, status=1 WHERE id_taikhoan=$idtaikhoan && status=0");
            $products = selectAll("SELECT * FROM ctdonhang WHERE id_donhang=$idDh");
            foreach ($products as $product) {
                $itemUpdate = "SELECT * from sanpham WHERE id = " . $product['id_sanpham'];
                foreach (selectAll($itemUpdate) as $item) {
                    if ($item['soluong'] > 0) {
                        $soluong = $item['soluong'] - $product['soluong'];
                        selectall("UPDATE sanpham SET soluong='$soluong' WHERE id={$item['id']}");
                    }
                };
            }
        ...
        if (isset($_POST["capnhatgiohang"])) {
            ...
            foreach ($soluong as $key => $itemSl) {
                $sql = "UPDATE `ctdonhang` SET `soluong`={$itemSl} WHERE id_donhang ={$idDh} AND id_sanpham ={$key}";
                exSQL($sql);
            }
